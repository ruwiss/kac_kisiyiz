...
function tokenRouter(router, root) {
    ...
    router.post("/auth", (req, res) => {
        ...
        root.con.getConnection((_, con) => {
            con.query(sql, (err, result) => {
                ...
                con.query(insertSql, values, (err, fields) => {
                    if (err) {
                        con.release();
        ...
        transporter.sendMail(mailOptions, function (error) {
            if (error) {
                ...
                root.con.getConnection((_, con) => {
                    ...
                    con.query("INSERT INTO resetpassword (mail, code) VALUES (?, ?)", [args.mail, code], (err) => {
                        if (err) {
                            con.release();
                            return helper.sendError(err, res);
                        }
    ...
    router.post("/verifyCode", (req, res) => {
        ...
        root.con.getConnection((_, con) => {
            con.query(sql, [args.mail], (err, result) => {
                if (err) {
                    con.release();
                    return helper.sendError(err, res);
                }
    ...
    router.post("/resetPassword", (req, res) => {
        ...
        root.con.getConnection((_, con) => {
            con.query(sql, [args.code], (err, result) => {
                if (err) {
                    con.release();
                    return helper.sendError(err, res);
                }
