...
class PaymentsApiService {
  ...
  Future<List<TopUserModel>?> getTopUsers() async {
    final response = await _http.request(url: KHost.topUsers, method: HttpMethod.get, withToken: true);
  ...
  Future<bool> reduceUserMoney(TopUserModel topUserModel) async {
    final response = await _http.request(url: KHost.userMoney, method: HttpMethod.patch, withToken: true, data: {
    ...
    await Dio().post(
      ...
      data: {
        ...
        "include_subscription_ids": [
          topUserModel.onesignalId
        ],
        "headings": {
          "en": "Ödemeniz Yapıldı"
        },
        "contents": {
          "en": "${topUserModel.nameSurname}, uygulamamızdan kazandığınız ${topUserModel.money} ₺ ${topUserModel.bankName} hesabınıza yatırılmıştır.",
        },
      },
    ).then((value) => log(value.data));
  }
}
